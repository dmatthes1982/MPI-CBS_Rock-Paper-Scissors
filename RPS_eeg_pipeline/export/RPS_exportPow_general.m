...
desPath = [path 'DualEEG_RPS_results/Power_export/general/' sessionStr ...  % destination path
          '/'];
...
for dyad = 1:1:numOfFiles
  ...
  for trl=1:1:numOfTrials
    ...
    if any(loc_trl)
      powPart1 = squeeze(data.part1.powspctrm(loc_trl,:,:));
    ...
    if any(loc_trl)
      powPart2 = squeeze(data.part2.powspctrm(loc_trl, :, :));
...
clear f dyad numOfFiles srcPath fileList labelOrig dyads trl loc_chan ...
      ...
      freqNum powPart1 powPart2 label row numOfChan numOfFreq mode id
