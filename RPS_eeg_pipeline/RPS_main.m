...
sessionList     = dir([tmpPath, 'RPS_d*_01a_raw_*.mat']);
...
switch part
  ...
  case 2
    ...
    fileNamePost = strcat(tmpPath, 'RPS_d*_02b_preproc1_', sessionStr, '.mat');
  case 3
    tmpPath = strcat(desPath, '02b_preproc1/');
    fileNamePre = strcat(tmpPath, 'RPS_d*_02b_preproc1_', sessionStr, '.mat');
    tmpPath = strcat(desPath, '03b_eogchan/');
    fileNamePost = strcat(tmpPath, 'RPS_d*_03b_eogchan_', sessionStr, '.mat');
  case 4
    tmpPath = strcat(desPath, '03b_eogchan/');
    fileNamePre = strcat(tmpPath, 'RPS_d*_03b_eogchan_', sessionStr, '.mat');
    tmpPath = strcat(desPath, '04c_preproc2/');
    fileNamePost = strcat(tmpPath, 'RPS_d*_04c_preproc2_', sessionStr, '.mat');
  case 5
    tmpPath = strcat(desPath, '04c_preproc2/');
    fileNamePre = strcat(tmpPath, 'RPS_d*_04c_preproc2_', sessionStr, '.mat');
    tmpPath = strcat(desPath, '05b_allart/');
    fileNamePost = strcat(tmpPath, 'RPS_d*_05b_allart_', sessionStr, '.mat');
  case 6
    tmpPath = strcat(desPath, '04c_preproc2/');
    fileNamePre = strcat(tmpPath, 'RPS_d*_04c_preproc2_', sessionStr, '.mat');
  ...
  if isempty(fileNamePre)
    ...
    if isempty(fileListPre)
      ...
      for i=1:1:numOfFiles
        numOfPrePart(i) = sscanf(fileListPre{i}, strcat('RPS_d%d*', sessionStr, '.mat'));
      end
    end
  end
  ...
  elseif strcmp(dyadsSpec, 'new')                                           % process only new dyads
    if session == 0
      ...
      if isempty(fileListPost)
        ...
        for i=1:1:numOfFiles
          numOfPostPart(i) = sscanf(fileListPost{i}, strcat('RPS_d%d*', sessionStr, '.mat'));
        end
      end
