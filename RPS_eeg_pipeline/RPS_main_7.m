...
[pbSpec(1:3).winLength]     = deal(1, 1, 1);
...
for i = numOfPart
  ...
  for j = 1:1:numel(pbSpec)
    ...
    fprintf(['<strong>Segmentation of Hilbert phase data at '...
              '%s (%g-%gHz).</strong>\n'], pbSpec(j).name, ...
              data_hilbert.bpFreq);
    data_hilbert  = RPS_segmentation( cfg, data_hilbert );
    ...
    if artifactRejection == true
      if artifactAvailable == true
        ...
        fprintf(['<strong>Artifact Rejection of Hilbert phase data at '...
                  '%s (%g-%gHz).</strong>\n'], pbSpec(j).name, ...
                  data_hilbert.bpFreq);
        data_hilbert = RPS_rejectArtifacts(cfg, data_hilbert);
        fprintf('\n');
      end
    end
    ...
    data_plv  = RPS_phaseLockVal(cfg, data_hilbert);
    data_mplv = RPS_calcMeanPLV(data_plv);
    clear data_hilbert
    ...
    cfg.sessionStr = sessionStr;
    RPS_writeTbl(cfg, data_plv);

    % export the PLVs into a *.mat file
    cfg             = [];
    cfg.desFolder   = strcat(desPath, '07a_plv/');
    cfg.filename    = sprintf('RPS_d%02d_07a_plv%s', i, ...
                                pbSpec(j).fileSuffix);
    cfg.sessionStr  = sessionStr;
    ...
    fprintf('%s ...\n', file_path);
    RPS_saveData(cfg, 'data_plv', data_plv);
    fprintf('Data stored!\n');
    clear data_plv
    ...
    cfg.filename    = sprintf('RPS_d%02d_07b_mplv%s', i, ...
                                pbSpec(j).fileSuffix);
    cfg.sessionStr  = sessionStr;
    ...
    fprintf('%s ...\n', file_path);
    RPS_saveData(cfg, 'data_mplv', data_mplv);
    fprintf('Data stored!\n\n');
    clear data_mplv
  end
  clear cfg_allart
