...
% Path settings
% -------------------------------------------------------------------------
path = '/data/pt_01843/eegData/DualEEG_RPS_processedDataOld/';
...
fileList     = dir([tmpPath, 'RPS_d*_01a_raw_*.mat']);
...
for i = 1:1:length(parts)
  ...
  fileList    = dir([tmpPath, ['RPS_d*' sessionStr '.mat']]);
  fileList    = struct2cell(fileList);
  fileList    = fileList(1,:);
  numOfFiles  = length(fileList);
  numOfPart   = zeros(1, numOfFiles);
  for j = 1:1:numOfFiles
    numOfPart(j) = sscanf(fileList{j}, strcat('RPS_d%d*', sessionStr, '.mat'));
  end
