% RPS_PERMTEST_MPLV - Using this script one can test if the difference of
% one connection (e.g. Cz-Cz) or one cluster of connections is significant
% between two phases. First, a paired-sample t-test is conducted. If
...
% null hypothesis says that we expect no difference between phases but
% a difference between dyads, the permutation is done only by randomly
% interchanging the values of the two phases within the dyads. It is
% highly recommended to only use this script, if the number of PLV segments
% is nearly the same in both phases for single dyads. Otherwise one
% should use RPS_PERMTEST_PLV
%
% See also RPS_PERMTEST_PLV
...
clear f cnt mPLVtemp tf dyad connections connMatrixBool dyads fileList ...
      passband label numOfFiles phase tmptrialinfo phaseNum condAcronym
